/*
    * VIEW 뷰) 
     : SELECT문을 통해 얻어진 결과물을 저장해둘 수 있는 객체 
        ...
        임시테이블과 같은 존재 (실제 데이터가 저장 되는게 아니라, 논리적으로만 저장되어 있음)
*/
...
-- 뷰를 생성할 수 있는 권한을 부여
GRANT CREATE VIEW TO C##KH; -- 관리자 계정으로...
...
/*
    * VIEW 옵션 * 
    ...
    - FORCE | NOFORCE
        ...
        * NOFORCE : 서브쿼리에 작성한 테이블이 존재해야지만 뷰를 성성 (기본값)
    - WITH CHECK OPTION : DEL(추가/수정/삭제) 사용 시 서브쿼리에 작성한 조건에 맞는 값으로만 실행되도록 하는 옵션
...
-- 뷰를 미리 생성만 해놓고 나중에 정의해서 쓰는 용도로 사용할 수 있따
