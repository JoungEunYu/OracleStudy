/*
    * PL/SQL : PROCEDURE LANGUAGE EXTENSION TO SQL
    ...
    SQL 문장 내에서 변수를 정의, 조건, 반복 등을 지원하여 SQL 단점을 보완
    다수의 SQL문을 한번에 실행가능
...
-- HELLO ORACLE 출력 : 화면에 출력하고자 할 때 DMBS_OUTPUT.PUT_LINE(출력할 내용)
BEGIN
    -- 자바 : System.out.prinln("Hello Java!");
    DBMS_OUTPUT.PUT_LINE('HELLO ORACLE');
END;
/ 
-- '/'까지를 하나의 프로시저로 보기 때문에 작성해주면 좋다.
-------------------------------------------------------------------------------
/*
    * 선언부 (DECLARE)
...
/*
    *  레퍼런스 타입 변수
        : 어떤 테이블의 어떤 컬럼의 데이터타입을 참조하여 해당 타읍으로 변수를 지정
...
BEGIN
     ...
     INTO EID, ENAME, SAL   -- 각 컬럼의 결과 값을 변수에 대힙
     FROM EMPLOYEE
     -- WHERE EMP_ID = 200;
...
BEGIN
    ...
    -- NULL 값인 경우 0으로 표기하기 위해 NVL 함수 사용가능
END;
/
---------------------------------------------------------------------------------
/*
    ...
    ** 조건문 **
        ...
        - IF/ELSE문 : IF 조건식 THEN 참일때실행내용 ELSE 거짓일때 실행할 내용 END IF;
        - IF/ELSIF 문 : IF 조건식1 THEN 실행내용1 ELSIF 조건식2 THEN 실행내용2 ... [ELSE 실행내용] END IF;
        
        - CASE/WHEN/THEN 문
            CASE 비교대상 WHEN 동등비교값 1 THEN 결과값1
                         WHEN 비교값2 THEN 결과값2
                         WHEN 비교값3 THEN 결과값3
                         ...
                         ELSE 결과값 N
...
*/
-- 사용자에게 숫자를 입력받아 10으로 나눈 결과를 출력. (0이 입력되었을 떄는 오류가 발생될 수 있다..)
DECLARE
    N NUMBER;
BEGIN
    N := &숫자;
...
BEGIN
    ...
    EXCEPTION
    WHEN TOO_MANY_ROWS THEN DBMS_OUTPUT.PUT_LINE('요구된 것보다 많은 수의 행');
    WHEN NO_DATA_FOUND THEN DBMS_OUTPUT.PUT_LINE('입력한 사수사번을 가진 사원이 없습니다.');
    WHEN OTHERS THEN DBMS_OUTPUT.PUT_LINE('오류발생. 관리자에게 문의 하셍');
END;
/