...
INSERT INTO MEMBER VALUES (1, 'wjddmsdb', '1234', '정은유', '남', '010-7777-7779', 'wjddmsdb@naver.com', SYSDATE);
...
INSERT INTO MEMBER_NOTNULL
    VALUES (1, 'DAWUN', '1234', '기다운', '남', '010-0101-0101', 'DAWUN@GMIL.COM', SYSDATE);
INSERT INTO MEMBER_NOTNULL
...
INSERT INTO MEMBER_NOTNULL
    VALUES (1, 'DAWUN', '1234', '기다운', '남', '010-0101-0101', 'DAWUN@GMIL.COM', SYSDATE);
--> 중복되는 데이터가 있음에도 추가가 잘 되었다.... (꼰란ㄷ하다ㅣ)
---------------------------------------------------------------------------------
/*
    UNIQUE 제약 조건 
...
INSERT INTO MEMBER_UNIQUE
    VALUES (1, 'KI', '1234', '기다운', '남', '010-1234-1234', 'BAJ@NAVER.COM', SYSDATE);
INSERT INTO MEMBER_UNIQUE
    VALUES (2, 'KI', '12', '기릿', '여', NULL, NULL, NULL);
...
INSERT INTO MEMBER_UNIQUE
    VALUES(1, 'KQ', '1234', '기다', '남', '010-1234-1234', 'HI@NATE.COM', SYSDATE);
INSERT INTO MEMBER_UNIQUE
    VALUES(2, 'QWER', '1234', 'QWER', '여', NULL, NULL, NULL);
INSERT INTO MEMBER_UNIQUE
    VALUES(3, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
INSERT INTO MEMBER_UNIQUE
    VALUES(4, 'KQ', '1234', '라라', '여', NULL, NULL, NULL);
...
--> 회원번호(MEM_NO)는 동일하나 회원아이디(MEM_ID)값이 다르기 때문에 추가됨
...
/*
    ...
    => 참조할 컬럼명을 생략할 경우 참조하는 테이블의 기본키 컬러밍 매칭됨
*/
-- MEMBER 테이블 삭제
DROP TABLE MEMBER;
...
INSERT INTO MEMBER
    VALUES (1, 'HI', '1234', '최종군', '남', SYSDATE, 100);
INSERT INTO MEMBER
    VALUES (2, 'BYE', '1234', '조건웅', '남', SYSDATE, 200);
INSERT INTO MEMBER
    VALUES (3, 'NIHAO', '1234', '임현호', '남', SYSDATE, 300);
INSERT INTO MEMBER
    VALUES (4, 'HAHA', '1234', '똥', '남', SYSDATE, NULL);
-- 외래키로 설정된 컬럼에는 기본적으로 NULL 값은 저장 가능
INSERT INTO MEMBER
    VALUES (5, 'LULLUL', '1234', '이주원', '여', SYSDATE, 100);
--> 오류  발생! "부모키를 찾을 수 없다" -> 회원등급 테이블에 저장되어 있지 않은 값을 사용
...
WHERE GRADE_NO = 100;
--> 자식 테이블(MEMBER)에서 100이라는 값을 사용하고 있기 때문에 삭제 불가

DELETE FROM MEMBER_GRADE
WHERE GRADE_NO = 300;
--> 자식 테이블(MEMBER)에서 300이라는 값을 사용하고 있지 않았다면 삭제 가능

-- 자식테이블에 이미 사용하고 있는 값이 있을 경우
-- 부모테이블로부터 무조건 삭제가 되지 않는 "삭제 옵션"이 있다,
...
/*
    ...
    - ON DELETE CASCADE : 부모테이블에 있는 데이터를 삭제 시 해당 데이터를 사용중인 자식테이블의 값도 삭제
*/
-- MEMBER 테이블 삭제
DROP TABLE MEMBER;
...
INSERT INTO MEMBER
    VALUES (2, 'BYE', '1234', '조건웅', '남', SYSDATE, 200);
INSERT INTO MEMBER
    VALUES (3, 'NIHAO', '1234', '임현호', '남', SYSDATE, NULL);
    
SELECT * FROM MEMBER;

-- 100번 등급에 대한 데이터 삭제 MEMGER-GRADE
DELETE FROM MEMBER_GRADE
WHERE GRADE_NO = 100;
--> 자식테이블(MEMBER)에서 100번 등급이 사용된 데이터가 NULL로 변경되면
-- 부모 테이블(MEMBER_GRADE)에서는 데이터가 잘 삭제되ㅓㅇㅆ따.
...
DELETE FROM MEMBER_GRADE
WHERE GRADE_NO = 100;
--> 자식 테이블에서 100번이 사용된 행 (데이터) 자체가 삭제되었음
--------------------------------------------------------------------------------
/*
    DEFAULT
    : 제약조건은 아님...
    컬럼을 선정하지 않고 데이터 추가시 NULL값이 아닌 기본적인 값을 저장하고자 할때
*/
...
/*
    * ALTER TABLE : 테이블에 변경사항을 적용하고자 할 때 사용
    ...
    ---
    - DEFAULT : ALTER TABLE 테이블명 MODIFY 컬럼명 DEFAULT 기본값;
*/
...
ALTER TABLE EMPLOYEE ADD FOREIGN KEY(JOB_CODE) REFERENCES JOB; -- 기본키는 생략가능
--------------------------------------------------------------------------------
