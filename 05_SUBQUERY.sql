...
/*
    * 서브쿼리의 종류 *
        서브 쿼리를 수행한 결과값이 몇행 몇열로 나오냐에 따라 분류
...
SELECT EMP_NAME "불쌍한 명수", SALARY "편의점보다 적을듯", HIRE_DATE "14년을 일했는데.."
FROM EMPLOYEE
...
/*
    ...
    > ANY (서브쿼리) : 여러 개의 결과갑 중에서 하나라도 큰 경우가 있으면 조회
...
-- 보너스 포함 연봉이 NULL이 아니여야 하고 , 보너스 포함 연봉이 3000만원 이상인 사원들만 조회
...
-- 2) 부서별 평균 급여가 270만원을 초과하는 부서에 해당하는 부서코드, 부서별 총 급여합, 
-- 부서별 평균 급여, 부서별 사원 수를 조회
